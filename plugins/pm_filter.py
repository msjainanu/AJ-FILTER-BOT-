...
async def next_page(bot, query):
    ...
    btn.insert(0,[
        InlineKeyboardButton("🗣️ SelecT ʟᴀɴɢᴜᴀɢᴇs 🗣️", callback_data=f"languages#{key}#{offset}#{req}")
        ])
    btn.insert(0,[
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [[
            InlineKeyboardButton(' ♦️ Mᴀɪɴ Cʜᴀɴɴᴇʟ ♦️', url=f'https://t.me/anurino_maingroupp'),
            InlineKeyboardButton('🔎 Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🔍', url=f'https://t.me/searchmoviehere')  
        ],[
            InlineKeyboardButton('📸 ᴛ-ɢʀᴀᴘʜ', callback_data='telegraph'),
            InlineKeyboardButton('🆎️ ғᴏɴᴛ', callback_data='font')    
        ],[ 
            InlineKeyboardButton('🏡 Hᴏᴍᴇ 🏡', callback_data='start'),
            InlineKeyboardButton('Cᴏɴᴛᴀᴄᴛ 👮', url=f'https://t.me/Jain_shabb_bot')
        ]] 
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(                     
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        if total_results >= 3:
            btn.insert(0,[
                InlineKeyboardButton("🗣️ SelecT ʟᴀɴɢᴜᴀɢᴇs  🗣️", callback_data=f"languages#{key}#0#{req}")
            ])
            btn.insert(0,[
        InlineKeyboardButton("‼️ Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ Mᴏᴠɪᴇ ‼️", url=f'https://t.me/how_to_download_watch_88')
        ])
